...
import aiohttp
import asyncio
from Clients.deepgram_client import deepgram_client
...
async def stream(websocket: WebSocket):
    ...
    def on_message(self, result, **kwargs):
        nonlocal is_playing, playback_task, cancel_event

        sentence = result.channel.alternatives[0].transcript
        if sentence:
            # If caller is speaking while bot is talking, stop playback
            if is_playing and not result.is_final:
                print("⚠️ Caller is interrupting. Cancelling playback.")
                cancel_event.set()
                if playback_task:
    ...
    def on_utterance_end(self, utterance_end, **kwargs):
        ...
        gpt_response = query_rephrase(str(segment), stream_sid)
        print(f"🤖 GPT(Q): {gpt_response}")
        ...
        playback_task = asyncio.run_coroutine_threadsafe(
            stream_audio_to_twilio(gpt_response, websocket, cancel_event),
            loop
        )
...
async def stream_audio_to_twilio(text: str, websocket: WebSocket, cancel_event: asyncio.Event):
    ...
    if cancel_event.is_set():
        print("🚫 Playback cancelled before sending ElevenLabs request.")
        return
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=payload) as response:
            if response.status == 200:
                ...
                if cancel_event.is_set():
                    print("🚫 Playback cancelled after receiving ElevenLabs response.")
                    return
                ...
                if cancel_event.is_set():
                    print("🚫 Playback cancelled during conversion.")
                    return

                mulaw_audio = audioop.lin2ulaw(pcm_data, 2)
                ...
                if websocket and not cancel_event.is_set():
...
if __name__ == "__main__":
    ...
    uvicorn.run("interrupt:app", port=port)
