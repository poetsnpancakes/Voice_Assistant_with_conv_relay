...
async def stream(websocket: WebSocket):
    ...
    def on_message(self, result, **kwargs):
        ...
        if sentence:
          ...
          if state["bot_speaking"] and tts_task and not tts_task.done():
            ...
            async with session.post(url, headers=headers, json=payload) as response:
                    ...
                    try:
                        while not response.content.at_eof():
                            if state["interrupted"].is_set():
                                print("⛔ Interruption triggered — stopping reader")
                                break
                ...
                async def writer():
                    try:
                        while True:
                            if state["interrupted"].is_set():
                                print("⛔ Interruption triggered — stopping writer")
                                break
                ...
                await asyncio.shield(asyncio.gather(reader(), writer()))
...
if __name__ == "__main__":
    ...
    uvicorn.run("temp:app", port=port)
